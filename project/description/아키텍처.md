...
flowchart TB
    ...
    LLMM --> VER
    OFF --> VER
    VER["🔍 답변 사후 검증 게이트 — src/verify/<br/>수치·날짜·단위·방향 한정어 ↔ 근거 대조<br/>+ 폐지본 인용 감지"] -->|"답변+출처+도구트레이스<br/>+검증 결과+PII 리포트"| API
    API --> U

    OBS["📈 관측성 — src/observability.py<br/>Trace/Span · 구조화 로그"] -.모든 스텝 계측.-> AGENT
```
...
3. **나가는 경계 — 근거 강제**: 모든 답에 출처가 붙고, 근거가 약하면 회피(abstention)하고,
   나가는 답변의 수치·날짜·방향 한정어는 사후 검증 게이트가 근거와 대조한다.
