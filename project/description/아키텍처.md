...
flowchart TB
    ...
    PRE["🛫 배포 전 점검 — src/preflight.py<br/>설정 불변식·코퍼스/폐지 체인·업무데이터 스키마<br/>+ 스모크 canary + 안전장치 자가 테스트<br/>(검증 게이트 · PII 마스킹)"] -. "기동 전 차단 게이트<br/>(run.sh · CI)" .-> API
```
...
| 스모크(canary) | 대표 질의가 정답 문서를 1위로 회수하는가, 대표 케이스가 인지일+15일로 트리아지되는가, **안전장치 자가 테스트** — 검증 게이트(심은 오류가 실제로 걸리는가)와 PII 마스킹(심은 개인정보가 실제로 지워지는가) |
