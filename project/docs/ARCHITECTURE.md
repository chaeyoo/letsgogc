...
| **관측성** | 스텝별 지연·성패를 span 트레이스+구조화 로그로 기록(`observability.py`), 응답에 `trace`·`latency_ms` 노출. 검증 게이트 **경고율 계기판**(`GateStats` → `/health`) + 응답 단위 **감사 로그**로 alert fatigue 조기 신호를 상시 측정 |
| **검증성** | pytest 156케이스(**불변식/fuzz** 포함) + eval(검색·신뢰성·**PV 워크플로**·**검증기 메타 평가**)을 CI에서 매 푸시 실행, 소표본 지표에 95% 신뢰구간 병기 |
| **답변 사후 검증** | 모든 응답이 통과하는 런타임 게이트(`src/verify/`) — 답변 속 수치·날짜·단위(근무일≠일)·고유어 수사(보름=15일)·**방향 한정어(이내↔이후)**·**날짜 역할(기한↔인지일 스왑)**을 '검색 근거 ∪ 결정론적 도구 출력 − 질문 에코'와 대조, 폐지본 인용 감지. 실패는 차단이 아니라 시끄러운 경고(본문·API·UI) |
| **배포 전 점검** | `src/preflight.py` — 설정 불변식·코퍼스/**폐지 체인** 무결성·업무데이터 스키마·스모크 canary + **안전장치 자가 테스트**(검증 게이트·PII 마스킹)를 기동 전에 강제(실패 시 exit 1, `run.sh`·CI 연동). '부팅은 되는데 운영 중 오답'이라는 실패 방향을 배포 시점에 차단 |
