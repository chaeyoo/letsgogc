...
| **검증성** | pytest 130케이스(**불변식/fuzz** 포함) + eval(검색·신뢰성·**PV 워크플로**·**검증기 메타 평가**)을 CI에서 매 푸시 실행, 소표본 지표에 95% 신뢰구간 병기 |
...
| **배포 전 점검** | `src/preflight.py` — 설정 불변식·코퍼스/**폐지 체인** 무결성·업무데이터 스키마·스모크 canary + **검증 게이트 자가 테스트**를 기동 전에 강제(실패 시 exit 1, `run.sh`·CI 연동). '부팅은 되는데 운영 중 오답'이라는 실패 방향을 배포 시점에 차단 |
