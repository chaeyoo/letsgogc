...
| **검증성** | pytest 118케이스(**불변식/fuzz** 포함) + eval(검색·신뢰성·**PV 워크플로**·**검증기 메타 평가**)을 CI에서 매 푸시 실행, 소표본 지표에 95% 신뢰구간 병기 |
| **답변 사후 검증** | 모든 응답이 통과하는 런타임 게이트(`src/verify/`) — 답변 속 수치·날짜·단위(근무일≠일)·고유어 수사(보름=15일)·**방향 한정어(이내↔이후)**를 '검색 근거 ∪ 결정론적 도구 출력 − 질문 에코'와 대조, 폐지본 인용 감지. 실패는 차단이 아니라 시끄러운 경고(본문·API·UI) |
