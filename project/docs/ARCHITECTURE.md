...
2. **2차 (리랭킹 v3):** 4신호 가중합 `(본문 커버리지(idf^p 가중) + 정확 구문 매칭 +
   섹션제목 매칭 + 문서제목 정합)`에 **섹션 타입 prior** 를 더해 재점수해 상위 N(=3)만
   남긴다. Cross-Encoder 리랭커의 경량 근사. 섹션 신호와 제목 신호를 분리한 것은
   BM25F처럼 필드별 매칭을 따로 보는 설계 — 같은 문서 안에서 '정답 섹션'을
   고르는 신호(섹션)와 하드네거티브 문서를 거르는 반증 신호(제목)의 역할이 다르다.
   섹션 타입 prior 는 **질의 의도로 게이트되는 구조 신호**: "X와의 차이(주의)" 대조
   섹션은 X 도메인 어휘를 통째로 인용해 어휘 신호로 못 거르므로 비교 질의가 아닐 때
   감점하고(-0.3), "목적/개요" 서두 섹션은 주제 어휘 요약으로 과대평가되므로 정의
   질의가 아닐 때 소폭 감점한다(-0.055, sweep 유효 밴드 0.04~0.07의 중앙).
...
- **1단계 회수에 전 가중으로 적용**하고, 2단계 리랭킹은 원 질의 토큰 기준으로
  ...
  리랭커가 판별력을 잃지 않게 한다. Hit@1 0.875(벡터만)→1.000(전체 파이프라인, 32문항 eval 검증).
...
| **버전 안전성** | 폐지(superseded) 구판 자동 제외 · `as_of` 시점 조회는 '그 시점의 현행' 반환(당시 시행 중이던 폐지본 포함) — 폐지 체인 시행일 단조성은 preflight 가 보증 |
...
| **관측성** | 스텝별 지연·성패를 span 트레이스+구조화 로그로 기록(`observability.py`), 응답에 `trace`·`latency_ms` 노출. 검증 게이트 **경고율 계기판**(`GateStats` → `/health`, `warn_rate_checked` 병기 — 회피·무클레임 응답의 분모 혼합 착시 제거) + 응답 단위 **감사 로그**로 alert fatigue 조기 신호를 상시 측정 |
| **검증성** | pytest 193케이스(**불변식/fuzz** 포함) + eval(검색·신뢰성·**PV 워크플로**·**검증기 메타 평가**)을 CI에서 매 푸시 실행, 소표본 지표에 95% 신뢰구간 병기 |
| **답변 사후 검증** | 모든 응답이 통과하는 런타임 게이트(`src/verify/`) — 답변 속 수치·날짜·단위(근무일≠일)·고유어 수사(보름=15일)·부분 날짜("7월 25일")·연도 표기("2025년")·**방향 한정어(이내↔이후 — 기간·날짜·고유어 표기 모두, 충돌 판정은 strict 계층 — 케이스 에코가 방향 경고를 무력화하지 못함)**·**날짜 역할(기한↔인지일 스왑)**을 '검색 근거 ∪ 결정론적 도구 출력 − 질문 에코'와 대조(**2계층 신뢰 소스**: 케이스 서술 에코에서만 지지되는 클레임은 from_case 라벨), 폐지본 인용 감지(**문서 단위 허용** — 이력 검색이 실제 반환한 문서만 면제). 실패는 차단이 아니라 시끄러운 경고(본문·API·UI) |
| **배포 전 점검** | `src/preflight.py` — 설정 불변식·코퍼스/**폐지 체인** 무결성·업무데이터 스키마·스모크 canary + **안전장치 자가 테스트**(검증 게이트의 **모든 경고 축** + PII 마스킹)를 기동 전에 강제(실패 시 exit 1, `run.sh`·CI 연동). '부팅은 되는데 운영 중 오답'이라는 실패 방향을 배포 시점에 차단 |
| **코딩 신뢰 계층** | 확정(검수 사전)/후보(LLT 참조, 사람 확정 대기)/미코딩 감지 3계층(`pv/coding.py`) — 자동 확정 금지로 집계 오염 차단, 코딩 실패가 보고요건 오판으로 연쇄되지 않음 |
...
### 5.2 버전 인지 검색 (`retriever._candidate_indices`)
규제문서 frontmatter의 `version`·`effective_date`·`status`를 청크로 전파하고,
세 검색 모드(벡터/하이브리드/리랭킹)가 **공통 후보 필터**를 거치게 해 폐지본을 일관되게 제외한다.
개정 이력이 필요한 경우 `include_superseded`로 구판까지 노출. `as_of` 시점 조회는
**'그 시점의 현행'을 반환**한다 — 폐지본이라도 [시행일, 후속본 시행일) 구간이면 당시
현행이므로 포함하고(폐지 필터를 그대로 겹치면 개정된 규정의 시점 조회가 0건이 되는
조합 결함의 수정), 시행일을 해석할 수 없는 문서는 fail-closed 로 제외한다. 시행일은
인덱싱 시 1회 파싱한다(핫패스 재파싱 금지).
