...
def test_redact_skips_kinship_relations():
    """친족·관계 호칭(부모·아드·며느 등)은 이름으로 오마스킹하지 않는다. (v10)

    "환자의 부모님이 대신 신고", "아드님이 투약"의 '부모'·'아드'는 이름이
    아니라 PV 최소보고요건의 보고자-환자 '관계' 정보인데, v9 의 스톱리스트
    반전(허용집합→스톱리스트) 이후 룩어헤드가 없어져 [이름] 으로 오마스킹됐다
    — 직함(교수님)과 정확히 같은 등급의 오탐인데 직함만 방어되고 친족 클래스는
    빠진 비대칭. 감사 리포트 왜곡 + 관계 정보 소실이라 오탐도 경계 결함으로
    센다(스톱리스트 확장은 유출을 늘리지 않는다 — 실명은 여전히 마스킹)."""
    r = redact("환자의 부모님이 대신 신고했고 아드님이 투약, 며느님과 이모님이 동행했습니다")
    assert not r.redacted
    assert "[이름]" not in r.text
    # 실명은 여전히 마스킹된다(스톱리스트 확장이 유출 방향으로 새지 않는지 대조)
    assert redact("김철수님이 호소").redacted
