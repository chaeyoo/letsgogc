...
>
> **GC녹십자가 공개한 `Hey.GC 2.0` 방향성(Agentic AI + MCP 사내 시스템 통합)과 같은 구조 원리**로 설계했다.
...
이것이 GC가 `Hey.GC 2.0`으로 사내에 하려는 방향과 같은 구조다.
...
| **테스트/CI** | pytest 235케이스(**불변식/fuzz 테스트** 포함) + GitHub Actions(**preflight**+테스트·검색/신뢰성/PV/검증기 평가 회귀) | `tests/`, 저장소 루트 `.github/workflows/ci.yml` |
...
```bash
cd project
./run.sh          # venv 생성 + 의존성 설치 + 배포 전 점검(preflight) + 서버 실행
...
### 기타
```bash
.venv/bin/pip install -r requirements-dev.txt   # 테스트 의존성(pytest)
.venv/bin/python -m src.preflight           # 배포 전 점검(설정·코퍼스·업무데이터·스모크+안전장치 자가 테스트(검증 게이트·PII 마스킹))
.venv/bin/python -m pytest                  # 전체 테스트 235케이스(불변식/fuzz 포함)
...
> ⚠️ **읽기 전 규율 — 아래 32문항은 튜닝에 사용한 '개발셋'이다.** 실패 문항을 분석해 신호를
> 추가하고 같은 문항으로 재측정하는 오류 분석 루프를 돌렸으므로, 이 표의 **1.000은 일반화
> 성능이 아니라 '개발셋 포화'다**(abstention 문턱의 순환성 주의와 같은 규율을 검색 수치에도
> 적용). 일반화의 근사는 아래 **홀드아웃**(튜닝 미사용 6문항) 수치로 따로 본다.
...
- **홀드아웃 Hit@1 0.833** [0.436, 0.970] (n=6, `eval/holdout_dataset.json` — 튜닝에 일절 사용하지
  않는 문항, 규율은 파일 `_comment`): 실패 1건은 "품목허가 전 제조소 GMP 확인" 질의가 REG-003 대신
  유사문서 REG-011(GMP 실태조사)로 가는 하드네거티브형 혼동이다. **개발셋 1.000과 이 0.833의 갭이
  '튜닝된 수치'와 '일반화'의 거리**이며, 이 문항을 고치는 튜닝을 하면 홀드아웃이 아니게 되므로
  수치를 그대로 둔다(고치려면 개발셋으로 옮기고 새 홀드아웃 문항으로 교체).
...
└── tests/                     # ✅ pytest 235케이스(retriever·버전·임베더·에이전트·MCP·PV·검증기·preflight·불변식/fuzz 회귀)
...
- **검증 가능:** 모든 주장을 `pytest`(235케이스 — **불변식/fuzz 테스트** 포함)와 `eval` 5종
