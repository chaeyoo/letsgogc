...
| **테스트/CI** | pytest 193케이스(**불변식/fuzz 테스트** 포함) + GitHub Actions(**preflight**+테스트·검색/신뢰성/PV/검증기 평가 회귀) | `tests/`, `.github/workflows/ci.yml` |
...
pytest                                       # 전체 테스트 193케이스(불변식/fuzz 포함)
...
├── tests/                     # ✅ pytest 193케이스(retriever·버전·임베더·에이전트·MCP·PV·검증기·preflight·불변식/fuzz 회귀)
...
- **검증 가능:** 모든 주장을 `pytest`(193케이스 — **불변식/fuzz 테스트** 포함)와 `eval` 5종
