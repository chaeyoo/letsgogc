...
| **PV 도메인 도구** | 이상사례 **트리아지 → 인과성(WHO-UMC) 제안 → 용어 코딩(MedDRA 방식) → ICSR 보고서 초안(최소보고요건 검증)** 전 워크플로(규칙 기반+근거 부착) | `src/pv/` |
...
| **테스트/CI** | pytest 65케이스 + GitHub Actions(테스트·평가 회귀) | `tests/`, `.github/workflows/ci.yml` |
...
├── tests/                     # ✅ pytest 65케이스(retriever·버전·임베더·에이전트·MCP·PV)
...
- **검증 가능:** 모든 주장을 `pytest`(65케이스)와 `eval`(검색·신뢰성)로 **재현**할 수 있다.
