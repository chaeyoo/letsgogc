...
| **제약/바이오 산업 이해** | RA·PV 도메인 도구 — RA: 검색·마감·체크리스트 / PV: 트리아지→인과성→코딩(3계층)→ICSR 초안 + 라벨 22케이스 평가 | `src/pv/`, `eval/pv_eval.py` |
...
| **테스트/CI** | pytest 237케이스(불변식/fuzz 포함) + Actions(preflight+평가 4종 회귀) | `tests/`, 루트 `.github/workflows/ci.yml` |
...
품질 확인: `.venv/bin/python -m src.preflight`(배포 전 점검) · `-m pytest`(237케이스) ·
...
> 수치로 따로 본다 — 이 갭은 고치지 않고 그대로 둔다(고치는 순간 홀드아웃이 아니게 된다).

| 영역 | 지표 | 값 |
|---|---|---|
| 검색(개발셋 32) | Hit@1 (벡터만 → 전체 파이프라인) | 0.875 → **1.000** [0.893, 1.000] |
| 검색(개발셋 32) | HardNegHit@1 / ContextRecall | 0.786 → **1.000** / 0.781 → **0.969** |
| **검색(홀드아웃 6)** | **Hit@1 — 일반화 근사** | **0.833** [0.436, 1.000] (실패 1건 = REG-003↔REG-011 유사문서 혼동) |
