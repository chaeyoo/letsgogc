...
| **테스트/CI** | pytest 331케이스(불변식/fuzz 포함) + Actions(preflight+평가 4종 회귀) | `tests/`, 루트 `.github/workflows/ci.yml` |
...
품질 확인: `.venv/bin/python -m src.preflight`(배포 전 점검) · `-m pytest`(331케이스) ·
