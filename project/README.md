...
사후 검증 게이트를 통과**해 답변 속 수치·날짜·**방향 한정어("이내"↔"이후")**가
근거/도구 결과와 대조되고(6.4절), 폐지된 규정을 인용하면 경고가 붙는다.
...
| **답변 사후 검증** | 모든 응답이 통과하는 **런타임 검증 게이트**(수치·날짜·**단위(근무일≠일)·고유어 수사(보름=15일)·방향 한정어(이내↔이후)** ↔ 근거 대조, 폐지본 인용 감지, 질문 전제 에코 구분) + **검증기 자체의 메타모픽 평가**(변조 탐지 5축·오탐 감시 3축, 95% CI) | `src/verify/`, `eval/verify_eval.py` |
...
| **테스트/CI** | pytest 118케이스(**불변식/fuzz 테스트** 포함) + GitHub Actions(테스트·검색/신뢰성/PV/검증기 평가 회귀) | `tests/`, `.github/workflows/ci.yml` |
...
pytest                                       # 전체 테스트 118케이스(불변식/fuzz 포함)
...
→ 해석의 규율 — **'핀'과 '실측'을 구분해 읽는다.** 탐지율 5축은 '근거에 없는 값·방향'으로
...
탐지 표본도 v1(swap 7 + offset 10)에서 **v2(5축 71건)**로 늘었다 — '근무일' 단위 인식이
코퍼스 핵심 처리기한(120·75·60 근무일…)을 검증 범위로 편입시킨 효과가 절반, 신규 3축이 절반이다.
...
│   ├── verify/                # 🔍 답변 사후 검증(수치·날짜·단위·고유어·방향 한정어 ↔ 근거 대조 + 폐지본 인용 감지)
...
├── tests/                     # ✅ pytest 118케이스(retriever·버전·임베더·에이전트·MCP·PV·검증기·불변식/fuzz 회귀)
...
- **엔터프라이즈 감각:** 출처·**버전** 추적, 근거 기반(**abstention으로 환각 억제**), **PII 비식별화(외부 API 경계 보호)**,
  **답변 사후 검증 게이트(수치·날짜·단위·방향 한정어 대조 + 폐지본 인용 감지 — 나가는 방향의 경계)**,
...
- **검증 가능:** 모든 주장을 `pytest`(118케이스 — **불변식/fuzz 테스트** 포함)와 `eval` 5종
