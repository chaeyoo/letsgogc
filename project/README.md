...
| **배포 전 점검(FDE Day-0)** | `python -m src.preflight` — 설정 불변식·코퍼스/**폐지 체인** 무결성·업무데이터 스키마·스모크(canary)+**안전장치 자가 테스트**(검증 게이트·PII 마스킹). 실패 시 기동 차단(run.sh·CI 연동) | `src/preflight.py` |
...
| **테스트/CI** | pytest 136케이스(**불변식/fuzz 테스트** 포함) + GitHub Actions(**preflight**+테스트·검색/신뢰성/PV/검증기 평가 회귀) | `tests/`, `.github/workflows/ci.yml` |
...
pytest                                       # 전체 테스트 136케이스(불변식/fuzz 포함)
...
├── tests/                     # ✅ pytest 136케이스(retriever·버전·임베더·에이전트·MCP·PV·검증기·preflight·불변식/fuzz 회귀)
...
- **검증 가능:** 모든 주장을 `pytest`(136케이스 — **불변식/fuzz 테스트** 포함)와 `eval` 5종
