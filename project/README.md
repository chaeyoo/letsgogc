...
| **답변 사후 검증** | 모든 응답이 통과하는 **런타임 검증 게이트**(수치·날짜·**단위(근무일≠일)·고유어 수사(보름=15일)·방향 한정어(이내↔이후)·날짜 역할(인지일↔마감일 스왑)** ↔ 근거 대조, 폐지본 인용 감지, 질문 전제 에코 구분) + **검증기 자체의 메타모픽 평가**(변조 탐지 6축·오탐 감시 3축, 95% CI) | `src/verify/`, `eval/verify_eval.py` |
| **배포 전 점검(FDE Day-0)** | `python -m src.preflight` — 설정 불변식·코퍼스/**폐지 체인** 무결성·업무데이터 스키마·스모크(canary)+**검증 게이트 자가 테스트**. 실패 시 기동 차단(run.sh·CI 연동) | `src/preflight.py` |
| **운영 계기판(경고율)** | 검증 게이트 통과/경고를 축별 집계해 `/health` 노출 + 응답 단위 **감사 로그** — alert fatigue 의 조기 신호를 상시 측정 | `src/observability.py`, `src/api/main.py` |
...
| **테스트/CI** | pytest 130케이스(**불변식/fuzz 테스트** 포함) + GitHub Actions(**preflight**+테스트·검색/신뢰성/PV/검증기 평가 회귀) | `tests/`, `.github/workflows/ci.yml` |
...
.venv/bin/python -m src.preflight           # 배포 전 점검(설정·코퍼스·업무데이터·스모크+게이트 자가 테스트)
pytest                                       # 전체 테스트 130케이스(불변식/fuzz 포함)
...
→ 해석의 규율 — **'핀'과 '실측'을 구분해 읽는다.** 탐지율 6축은 '근거에 없는 값·방향·역할'로
...
탐지 표본도 v1(swap 7 + offset 10) → v2(5축 71건) → **v3(6축 76건)**로 늘었다 — v2는 '근무일'
...
- **배포 전(preflight, `src/preflight.py`)**: 서버 기동 전에 결정론적 점검 4그룹을 강제한다 —
  ...
  테스트**(근거 밖 수치를 심은 답변이 실제로 걸리는가 — 안전장치가 고장난 채 배포되는
  것은 없는 것보다 나쁘다). 실패 시 **exit 1로 기동 차단**(run.sh·CI 연동) — 운영 중
  '경고 부착'과 달리 배포 시점은 아직 사용자가 없으므로 시끄럽게 멈추는 비용이 0이다.
- **운영 중(계기판, `src/observability.py` GateStats)**: 검증 게이트의 통과/경고를 축별로
  집계해 `/health`에 노출하고(경고율 `warn_rate`), 응답 단위 **감사 로그**(판정 요약 JSON,
...
│   ├── preflight.py           # 🛫 배포 전 점검(설정·코퍼스/폐지 체인·업무데이터·스모크+게이트 자가 테스트 — 실패 시 기동 차단)
...
├── tests/                     # ✅ pytest 130케이스(retriever·버전·임베더·에이전트·MCP·PV·검증기·preflight·불변식/fuzz 회귀)
└── .github/workflows/ci.yml   # 🔁 CI(preflight+테스트+평가 회귀)
```
...
- **배치 운영 감각(FDE의 시간축):** 배포 전에는 **preflight 게이트**(데이터·설정·스모크·
  검증 게이트 자가 테스트 — 실패 시 기동 차단)가, 배치 후에는 **경고율 계기판**(`/health`)과
  응답 단위 **감사 로그**가 지킨다 — "부팅은 되는데 운영 중에 오답"이라는 최악의 실패
  방향과 "경고가 무시되기 시작하는 순간(alert fatigue)"을 둘 다 보이게 만들었다.
- **검증 가능:** 모든 주장을 `pytest`(130케이스 — **불변식/fuzz 테스트** 포함)와 `eval` 5종
