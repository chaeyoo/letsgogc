...
사후 검증 게이트를 통과**해 답변 속 수치·날짜가 근거/도구 결과와 대조되고(6.4절),
폐지된 규정을 인용하면 경고가 붙는다.
...
flowchart TB
    ...
    AG --> VER["🔍 답변 사후 검증<br/>(수치·날짜 ↔ 근거 대조<br/>+ 폐지본 인용 점검)"]
...
| **답변 사후 검증** | 모든 응답이 통과하는 **런타임 검증 게이트**(수치·날짜 ↔ 근거 대조, 폐지본 인용 감지) + **검증기 자체의 메타모픽 평가**(변조 탐지·오탐 감시) | `src/verify/`, `eval/verify_eval.py` |
...
| **테스트/CI** | pytest 102케이스(**불변식/fuzz 테스트** 포함) + GitHub Actions(테스트·검색/신뢰성/PV/검증기 평가 회귀) | `tests/`, `.github/workflows/ci.yml` |
...
pytest                                       # 전체 테스트 102케이스(불변식/fuzz 포함)
...
```
> CI: 매 푸시마다 GitHub Actions가 `pytest` + 네 평가(검색·신뢰성·PV·검증기)를 실행해 회귀를 막는다(`.github/workflows/ci.yml`).
...
- **수치·날짜 대조**: 답변에서 숫자+단위(15일·6개월·120일…)와 날짜(YYYY-MM-DD)를 추출해,
  **신뢰 소스 = 검색 근거 ∪ 결정론적 도구 출력** 안에 실제로 존재하는지 확인한다.
  (도구가 '계산해 낸' 마감일은 근거 원문에 없으므로, 근거만 보면 오탐이 된다 — 신뢰
  소스의 정의가 이 게이트의 핵심 설계다.)
...
**결정론적 변조**를 가해 오류 케이스를 합성하는 메타모픽 방식이다:

| 지표 | 값 | 의미 |
|---|---|---|
| CleanPassRate | **1.000** (n=32) | 정상(근거 발췌) 답변 통과율 — 오탐 감시(오탐↑ = alert fatigue = 계층 사망) |
| SwapDetection | **1.000** (n=7) | **교차문서 수치 치환**(의료기기 80일↔의약품 120일 같은 '그럴듯한 혼동') 탐지율 |
| OffsetDetection | **1.000** (n=10) | 오프셋 변조(15일→22일) 탐지율 |
| 폐지본 인용 감지 | ✓ (이력 모드 허용 ✓) | 버전 검증 축 |
| E2EPassRate | **1.000** (n=40) | 오프라인 에이전트 **실응답 전수**가 게이트를 통과 — 포매터가 근거 밖 수치를 만들면 깨진다 |

→ 탐지율은 '근거에 없는 값'으로 합성했으므로 1.0이 정상이며, **깨지는 순간이 곧 검증기
회귀**(클레임 추출·대조 로직)다 — 회귀 고정 핀. CleanPassRate·E2EPassRate는 실측이다.
...
│   ├── verify/                # 🔍 답변 사후 검증(수치·날짜 ↔ 근거 대조 + 폐지본 인용 감지)
...
│   └── api/main.py            # ⚙️ FastAPI(trace·latency·grounded·redactions·verification 노출)
...
├── tests/                     # ✅ pytest 102케이스(retriever·버전·임베더·에이전트·MCP·PV·검증기·불변식/fuzz 회귀)
...
- **엔터프라이즈 감각:** 출처·**버전** 추적, 근거 기반(**abstention으로 환각 억제**), **PII 비식별화(외부 API 경계 보호)**,
  **답변 사후 검증 게이트(수치·날짜 대조 + 폐지본 인용 감지 — 나가는 방향의 경계)**,
...
- **검증 가능:** 모든 주장을 `pytest`(102케이스 — **불변식/fuzz 테스트** 포함)와 `eval` 5종
