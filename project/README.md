...
| **답변 사후 검증** | 모든 응답이 통과하는 **런타임 검증 게이트**(수치·날짜(**한국어 표기 정규화** 포함)·**단위(근무일≠일)·고유어 수사(보름=15일)·방향 한정어(이내↔이후)·날짜 역할(인지일↔마감일 스왑)** ↔ 근거 대조(입력 에코·에러 계약 응답은 신뢰 소스에서 제외), 폐지본 인용 감지, 질문 전제 에코 구분(직전 턴 창)) + **검증기 자체의 메타모픽 평가**(변조 탐지 6축·오탐 감시 3축, 95% CI) | `src/verify/`, `eval/verify_eval.py` |
| **배포 전 점검(FDE Day-0)** | `python -m src.preflight` — 설정 불변식·코퍼스/**폐지 체인** 무결성(실존·현행 종점·**시행일 단조성**)·업무데이터 스키마·스모크(canary — RAG·PV·**시점 조회**)+**안전장치 자가 테스트**(검증 게이트·PII 마스킹). 실패 시 기동 차단(run.sh·CI 연동) | `src/preflight.py` |
...
| **테스트/CI** | pytest 156케이스(**불변식/fuzz 테스트** 포함) + GitHub Actions(**preflight**+테스트·검색/신뢰성/PV/검증기 평가 회귀) | `tests/`, `.github/workflows/ci.yml` |
...
pytest                                       # 전체 테스트 156케이스(불변식/fuzz 포함)
...
├── tests/                     # ✅ pytest 156케이스(retriever·버전·임베더·에이전트·MCP·PV·검증기·preflight·불변식/fuzz 회귀)
...
- **검증 가능:** 모든 주장을 `pytest`(156케이스 — **불변식/fuzz 테스트** 포함)와 `eval` 5종
