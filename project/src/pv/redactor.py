...
# 비대칭. 호칭 뒤 조사 시작 글자를 허용 집합으로 열되, 조사가 아닌 한글
# 연속(예: "도우미님들"의 스톱리스트 외 합성어)은 오탐 방향이므로
# 스톱리스트로 방어한다(마스킹은 과탐이 미탐보다 싸다).
_NAME_RE = re.compile(r"([가-힣]{2,4})\s?(님|씨)(?=$|[^가-힣]|[이가은는을를의과와도만께에한])")
_NAME_STOP = {
