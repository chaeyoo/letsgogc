...
# 같은 잔존 서술(임상 최빈 표현)에 오발화해 환자정보 없는 케이스의 ①요건을
# 조용히 충족시켰다(v11). 동사 활용(남아 있다/계시다/서/났다)이 뒤따르면
# 배제한다 — 명사 용법(남아가/남아는/남아 환자)은 유지.
_PATIENT_RE = re.compile(r"\d{1,3}\s*세(?!대)|[남여]성|남아(?!\s*(?:있|계|서|났|나서))|여아")
...
# 보고 행위가 결합된 형태만 인정한다.
# 한의사·한약사(v11) — F1(v10) 의 보고자 룩비하인드 `(?<![가-힣])의사`는 앞이
# 한글이면 비매칭이라, 한글에 직결된 정당 보고자 "한의사"(한+의사)·"한약사"
# (한+약사)를 놓쳤다(경계 봉합의 반대편 미탐). 이들은 KAERS 유효 보고자 자격이므로
# 마커로 명시 인정한다. 전문의·주치의는 애초에 '의사' 어절이 없어(전문+의·주치+의)
# v9 이전부터의 갭 — 함께 보강한다. (자격어 전수는 실무 사전 참조가 정석.)
