...
def smoke_checks() -> list[str]:
    ...
    #     v10 — 오탐(과잉 마스킹) 방향의 clean-case: 검증 게이트의 '정상 케이스
    #     통과'와 같은 원리로, 마스킹도 미탐만이 아니라 오탐으로 죽는다(감사
    #     리포트 왜곡 + 보고자-환자 관계 정보 소실). 직함(교수님)에 이어 친족·
    #     관계 호칭("부모님이"·"아드님이")도 이름으로 오마스킹되지 않는지 태운다
    #     — 스톱리스트가 특정 클래스에만 적용되는 비대칭을 자가 테스트로 고정.
    clean = redact("환자의 부모님이 대신 신고했고 아드님이 투약, 이모님이 동행했습니다")
    if clean.redacted:
        problems.append(
            f"PII 마스킹 자가 테스트 실패: 친족·관계 호칭이 이름으로 오마스킹됨(오탐) — {clean.text}"
        )
