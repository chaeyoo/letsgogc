...
├── eval/                       # 📊 평가 4종: evaluate(검색) · sweep(스윕/ablation) · faithfulness(신뢰성) · pv_eval(PV)
└── tests/                      # ✅ pytest 83케이스
```
...
**⑤ 벡터스토어 (`vectorstore.py`)** — 청크와 임베딩을 리스트로 보관하고 코사인 top-k를 제공하는 인메모리 구현. 실무의 Vector DB(pgvector/Qdrant) 자리의 경량 대체물. 코퍼스가 작아(13문서 → 수백 청크) 전수 비교로 충분하다.
...
여기에 `tests/` 83케이스가 회귀를 잡고, GitHub Actions CI가 매 푸시마다 pytest + 평가 3종을 실행한다.
...
2. **근거 없는 답은 내보내지 않는다** — 모든 답에 출처, 근거가 약하면 abstention, PV 판정에도 근거 규정(basis) 부착. 규제 도메인의 신뢰성 요건을 응답 구조 자체에 박았다.
3. **표준 규격으로 분리해 재사용** — 도구는 MCP로(클라이언트 무관), 임베더는 Protocol 인터페이스로(TF-IDF↔해싱↔Voyage 무수정 교체). 확장 지점이 말이 아니라 코드로 존재한다.
4. **모든 주장은 재현 가능한 수치로** — 4개 평가 스크립트 + 스윕/ablation + 83개 테스트 + CI. 하이퍼파라미터 하나까지 "왜 그 값인가"를 스크립트로 재현한다. 정직한 갭(ContextRecall 1건, 코딩 재현율 0.792)도 숨기지 않고 '다음 확장 지점의 크기'로 해석한다.
