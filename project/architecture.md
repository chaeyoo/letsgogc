...
└── tests/                      # ✅ pytest 102케이스(불변식/fuzz 포함)
```
...
여기에 `tests/` 102케이스(불변식/fuzz 포함)가 회귀를 잡고, GitHub Actions CI가 매 푸시마다 pytest + 평가 4종(검색·신뢰성·PV·검증기)을 실행한다.
...
2. **근거 없는 답은 내보내지 않는다** — 모든 답에 출처, 근거가 약하면 abstention, PV 판정에도 근거 규정(basis) 부착, 나가는 답변의 수치·날짜는 사후 검증 게이트가 근거와 대조. 규제 도메인의 신뢰성 요건을 응답 구조 자체에 박았다.
3. **표준 규격으로 분리해 재사용** — 도구는 MCP로(클라이언트 무관), 임베더는 Protocol 인터페이스로(TF-IDF↔해싱↔Voyage 무수정 교체). 확장 지점이 말이 아니라 코드로 존재한다.
4. **모든 주장은 재현 가능한 수치로** — 5개 평가 스크립트 + 스윕/ablation + 102개 테스트 + CI, 소표본 지표엔 95% 신뢰구간 병기. 하이퍼파라미터 하나까지 "왜 그 값인가"를 스크립트로 재현한다. 정직한 갭(ContextRecall 1건, 코딩 재현율 0.792)도 숨기지 않고 '다음 확장 지점의 크기'로 해석한다.
