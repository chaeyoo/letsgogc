...
sequenceDiagram
    ...
    Note over AG: ⑤ 답변 사후 검증 게이트 (src/verify)<br/>수치·날짜·방향·역할 ↔ 근거 대조 → 경고 부착
    AG-->>API: 근거 발췌 답변 + citations + trace + 검증 결과
    API-->>U: 답변 + 출처 + 도구 트레이스 + 지연(ms) + 검증 배지
```
...
sequenceDiagram
    ...
    loop 최대 6스텝 (무한루프 방지)
        ...
        alt stop_reason == "tool_use"
            ...
            Note over AG: 루프 종료 → 답변 사후 검증 게이트(src/verify)<br/>→ 답변 + 출처 + 검증 결과 반환
        end
    end
```
...
│   ├── verify/verifier.py      # 🔍 답변 사후 검증 게이트 (수치·날짜·방향·역할 ↔ 근거 대조, 경고 부착)
...
└── tests/                      # ✅ pytest 235케이스(불변식/fuzz 포함)
```
...
| 검증 | `src/verify/verifier.py` | 모든 응답의 수치·날짜·방향·역할을 근거와 대조하는 사후 검증 게이트 |
...
여기에 `tests/` 235케이스(불변식/fuzz 포함)가 회귀를 잡고, GitHub Actions CI가 매 푸시마다 **preflight(배포 전 점검)** + pytest + 평가 4종(검색·신뢰성·PV·검증기)을 실행한다.
...
4. **모든 주장은 재현 가능한 수치로** — 5개 평가 스크립트 + 스윕/ablation + 235개 테스트 + CI, 소표본 지표엔 95% 신뢰구간 병기. 하이퍼파라미터 하나까지 "왜 그 값인가"를 스크립트로 재현한다. 정직한 갭(ContextRecall 1건, 코딩 재현율 0.792)도 숨기지 않고 '다음 확장 지점의 크기'로 해석한다.
